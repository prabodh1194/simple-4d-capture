    ...
    private func setAlertsForCategory(reminder: EKReminder, category: Category) {
        ...
        case .doIt:
            ...
            if hour >= 9 {
                ...
                alertDate = calendar.date(
                    byAdding: .day,
                    value: 1,
                    to: calendar.date(bySettingHour: 9, minute: 0, second: 0, of: now)!
                )!
            } else {
                // Alert today at 9 AM
...
enum ReminderError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .listNotFound(name): return "List '\(name)' not found"
        case let .saveFailed(error): return "Failed to save: \(error.localizedDescription)"
        }
    }
}
